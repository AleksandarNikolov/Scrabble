...
public class Game {
	...
	public Game(ArrayList<String> queue) {
		...
		for (int i = 0; i < queue.size(); i++) {
			...
			Player player;
			players.add(player = new Player(i, queue.get(i)));
		}
	...
	public void place(String[] command, Player player)
		...
		if (checkWordInDeck(word, player)
			...
			// at the end we just multiply the added up score from all the letters with the
			// word multiplier
	...
	/**
	 * check the word can be made with words from the deck. specifically this checks
	 * if the tile that is going to be placed on the board is currently in the deck.
	 * 
	 * @param word
	 * @param player
	 * @return
	...
	public boolean checkWordInDeck(String word, Player player) {
		...
		// the counter is equal or greater than the letters.length that means all the
		// tiles needed to make the word exist and there could even be extra.
	...
	// check that the square where we wlll place a tile is not occupied by a tile
	...
	// if a tile is a usefull tile it means that it is already part of the word at
	// the correct position
	// so when placing the tiles, the specific tile does not need to be placed
	// again.
